...
flags.DEFINE_integer("max_steps", 100000, "Batch size for finetuning.")
flags.DEFINE_integer("save_per_steps", 10000, "Batch size for finetuning.")
flags.DEFINE_list("datasets",["sim_2env_transfer_cube_300","sim_2env_insertion_300"],"Dataset Names.")
...
def main(_):
    ...
    wandb.init(name="finetune_aloha", project="octo", mode="offline")
    ...
    assert (
        len(FLAGS.datasets) > 0
    ), "至少指定一个数据集"
    
    print(f"##### datasets={FLAGS.datasets}")
    
    if len(FLAGS.datasets)==1:
        dataset = make_single_dataset(
            dataset_kwargs=dict(
                #name="aloha_sim_cube_scripted_dataset",
                #name="sim_2env_transfer_cube_300",  
                #name="sim_2env_insertion_300", 
                #name=','.join(FLAGS.datasets),
                name=FLAGS.datasets[0],
                data_dir=FLAGS.data_dir,
                image_obs_keys={"primary": "top", "left_wrist":"left_wrist", "right_wrist":"right_wrist"},
    ...
    if image_input_mode==0:
        ...
        config["model"]["observation_tokenizers"]["left_wrist"]= ModuleSpec.create(
                ...
                task_stack_keys=["image_left_wrist"],
        ...
        config["model"]["observation_tokenizers"]["right_wrist"]= ModuleSpec.create(
                ...
                task_stack_keys=["image_right_wrist"],
                encoder=ModuleSpec.create(SmallStem16),
        )
    elif image_input_mode==2:
        del config["model"]["observation_tokenizers"]["wrist"]
        config["model"]["observation_tokenizers"]["all_wrists"]= ModuleSpec.create(
                ImageTokenizer,
                obs_stack_keys=["image_.*_wrist"],
                task_stack_keys=["image_.*_wrist"],
                encoder=ModuleSpec.create(SmallStem16),
            )
    elif image_input_mode==3:
        config["model"]["observation_tokenizers"]["wrist"]["kwargs"]["obs_stack_keys"]=["image_.*_wrist"]
        config["model"]["observation_tokenizers"]["wrist"]["kwargs"]["task_stack_keys"]=["image_.*_wrist"]
