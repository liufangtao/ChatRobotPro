...
class WorkspaceIL:
    def __init__(self, cfg):
        ...
        width = self.cfg.suite.task_make_fn.width
        height = self.cfg.suite.task_make_fn.height
        proprioceptive_len = 14 if self.cfg.expert_dataset.prop_use_joints else 18
        action_spect_len = 14 if self.cfg.expert_dataset.env_is_joints else 16

        # self.env, self.task_descriptions = hydra.utils.call(self.cfg.suite.task_make_fn)
        observation_spec={'top': np.ones(shape=(3, height,width)), 'left_wrist': np.ones(shape=(3, height,width)), 'right_wrist': np.ones(shape=(3, height,width)),'angle': np.ones(shape=(3, height,width)), 'proprioceptive': np.ones(shape=(proprioceptive_len,)),'features':np.ones(shape=(71,))}
        action_spec= np.ones(shape=(action_spect_len,))
        
        self.agent = make_agent(
            observation_spec, action_spec, cfg
        )
        # create agent
        # self.agent = make_agent(
