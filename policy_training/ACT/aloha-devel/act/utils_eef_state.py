...
class EpisodicDataset(torch.utils.data.Dataset):
    ...
    def __init__(self, episode_ids, dataset_dir, camera_names, images_depth,norm_stats, arm_delay_time,
    ...
    def __getitem__(self, index):
        ...
        with h5py.File(dataset_path, 'r') as root:
            ...
            original_action_shape = root['/master_arm_eef_state'].shape
            ...
            actions = root['/observations/arm_eef_state'][1:]
            actions = np.append(actions, actions[-1][np.newaxis, :], axis=0)
            ee = root['/observations/arm_eef_state'][start_ts]
            if self.use_robot_base:
                ee = np.concatenate((ee, root['/base_state'][start_ts]), axis=0)
            image_dict = dict()
            ...
            for cam_name in self.images_depth:
            ...
            if self.use_robot_base:
                action = np.concatenate((action, root['/base_state'][index:]), axis=1)
        ...
        image_data = torch.from_numpy(all_cam_images)  #图片数据
        ...
        if self.use_depth_image:
            ...
            # construct observations
            image_depth_data = torch.from_numpy(all_cam_images_depth.astype(np.float32))
...
def get_norm_stats(dataset_dir, num_episodes, use_robot_base):
    ...
    for episode_idx in range(num_episodes):
        ...
        with h5py.File(dataset_path, 'r') as root:
            ...
            # 从hdf5文件中读取action数据
            action = root['/master_arm_eef_state'][()]

            # 如果使用机器人基座数据
            if use_robot_base:
                # 拼接ee和基座动作数据
                ee = np.concatenate((ee, root['/base_state'][()]), axis=1)
                # 拼接action和基座动作数据
                action = np.concatenate((action, root['/base_state'][()]), axis=1)
...
def load_data(dataset_dir, num_episodes, arm_delay_time, use_depth_image,  # /1T/cobot_magic/aloha-devel/datas/ ,70, 0, False, 
              use_robot_base, camera_names,images_depth, batch_size_train, batch_size_val):  # False, ['cam_high', 'cam_left_wrist', 'cam_right_wrist'], 8, 8
    ...
    train_dataset = EpisodicDataset(train_indices, dataset_dir, camera_names, images_depth, norm_stats, arm_delay_time,
                                    use_depth_image, use_robot_base)

    val_dataset = EpisodicDataset(val_indices, dataset_dir, camera_names, images_depth, norm_stats, arm_delay_time,
                                  use_depth_image, use_robot_base)
