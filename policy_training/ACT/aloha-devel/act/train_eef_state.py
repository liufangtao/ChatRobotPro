...
from utils_eef_state import load_data 
from utils_eef_state import compute_dict_mean, set_seed, detach_dict
...
def train(args):
    ...
    TASK_CONFIGS = {
        args.task_name: {
            ...
            'camera_names': ['camera_3_rgb_front', 'camera_1_rgb_left', 'camera_2_rgb_right','camera_4_rgb_head'],
            'images_depth': ['camera_3_depth_front','camera_1_depth_left','camera_2_depth_right','camera_4_depth_head'],
    ...
    images_depth = task_config['images_depth']
    ...
    train_dataloader, val_dataloader, stats, _ = load_data(dataset_dir, num_episodes, args.arm_delay_time,
                                                           args.use_depth_image, args.use_robot_base, camera_names,images_depth,
...
def forward_pass(policy_config, data, policy):
    image_data, image_depth_data, ee_data, action_data, action_is_pad = data
    (image_data, ee_data, action_data, action_is_pad) = (image_data.cuda(), ee_data.cuda(),
    ...
    return policy(image_data, image_depth_data, ee_data, action_data, action_is_pad)


def train_process(train_dataloader, val_dataloader, config, stats):
    post_process = lambda a: a * stats['ee_std'] + stats['ee_mean']
...
def get_arguments():
    ...
    parser.add_argument('--dataset_dir', action='store', type=str, help='dataset_dir', default='/1T/datas/data20240723/_rename')
    parser.add_argument('--ckpt_dir', action='store', type=str, help='ckpt_dir', default='/1T/cobot_magic/aloha_devel/train0723')
    parser.add_argument('--num_episodes', action='store', type=int, help='num_episodes', default=6)
    parser.add_argument('--pretrain_ckpt', action='store', type=str, help='pretrain_ckpt', default='', required=False)
    parser.add_argument('--task_name', action='store', type=str, help='task_name', default='grab_coke_left', required=False)
