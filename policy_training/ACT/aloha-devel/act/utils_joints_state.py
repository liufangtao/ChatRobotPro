...
class EpisodicDataset(torch.utils.data.Dataset):
    ...
    def __getitem__(self, index):
        ...
        with h5py.File(dataset_path, 'r') as root:
            ...
            original_action_shape = root['/master_arm_joints_state'].shape
            ...
            actions = root['/observations/arm_joints_state'][1:]
            actions = np.append(actions, actions[-1][np.newaxis, :], axis=0)
            qpos = root['/observations/arm_joints_state'][start_ts]
            if self.use_robot_base:
                qpos = np.concatenate((qpos, root['/base_state'][start_ts]), axis=0)
            image_dict = dict()
...
def get_norm_stats(dataset_dir, num_episodes, use_robot_base):
    """
    从给定的数据集目录中读取多个episode的qpos和action数据，并计算其标准化统计信息。
    """
    all_qpos_data = []
    all_action_data = []
    ...
    for episode_idx in range(num_episodes):
        ...
        with h5py.File(dataset_path, 'r') as root:
            ...
            if use_robot_base:
                # 拼接qpos和基座动作数据
                qpos = np.concatenate((qpos, root['/base_state'][()]), axis=1)
                # 拼接action和基座动作数据
                action = np.concatenate((action, root['/base_state'][()]), axis=1)

        # 将qpos数据转换为torch张量并添加到列表中
