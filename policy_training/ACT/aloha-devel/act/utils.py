...
class EpisodicDataset(torch.utils.data.Dataset):
    ...
    它初始化了数据集的特定配置，如场景ID、数据集目录、相机名称、标准化统计数据，
    以及是否使用深度图像和机器人基座信息等。
    
    参数:
    - episode_ids: 场景ID列表，指示数据集包含的场景。
    - dataset_dir: 数据集目录路径，存储所有场景的数据。
    - camera_names: 相机名称列表，指示数据集中使用的相机视角。
    - norm_stats: 标准化统计数据，用于图像数据的预处理。
    - arm_delay_time: 机械臂动作延迟时间，用于模拟现实世界中机械臂的延迟。
    - use_depth_image: 布尔值，指示是否使用深度图像。
    - use_robot_base: 布尔值，指示是否使用机器人的基座信息。
    """
    ...
    def __init__(self, episode_ids, dataset_dir, camera_names, norm_stats, arm_delay_time,
        ...
        初始化数据集实例。
        """
        super(EpisodicDataset).__init__()  # 调用父类的初始化方法
        self.episode_ids = episode_ids  # 存储场景ID列表
        self.dataset_dir = dataset_dir  # 存储数据集目录路径
        self.camera_names = camera_names  # 存储相机名称列表
        self.norm_stats = norm_stats  # 存储图像标准化统计数据
        self.is_sim = None  # 初始化一个标志，用于后续判断是否为模拟环境
        self.use_depth_image = use_depth_image  # 标记是否使用深度图像
        self.arm_delay_time = arm_delay_time  # 存储机械臂的延迟时间
        self.use_robot_base = use_robot_base  # 标记是否使用机器人基座信息
        self.__getitem__(0)  # 通过访问第一个元素来初始化self.is_sim
...
def get_norm_stats(dataset_dir, num_episodes, use_robot_base):
    """
    从给定的数据集目录中读取多个episode的数据，并计算并返回动作和qpos的统计数据。
    
    Args:
        dataset_dir (str): 数据集目录的路径。
        num_episodes (int): 要从数据集中读取的episode数量。
        use_robot_base (bool): 是否使用机器人的基础动作和位置数据。
    
    Returns:
        dict: 包含以下键的字典：
            - action_mean (np.ndarray): 动作数据的均值。
            - action_std (np.ndarray): 动作数据的标准差。
            - qpos_mean (np.ndarray): qpos数据的均值。
            - qpos_std (np.ndarray): qpos数据的标准差。
            - example_qpos (np.ndarray): 一个示例的qpos数据（仅作为参考，不用于计算统计量）。
