...
import os
import sys
ROBOMIMIC_MODULE_PATH = os.path.abspath(".")
ROBOMIMIC_MODULE_PATH = ROBOMIMIC_MODULE_PATH + "/aloha-devel"
if ROBOMIMIC_MODULE_PATH not in sys.path:
    sys.path.append(ROBOMIMIC_MODULE_PATH)
    print(sys.path)
sys.path.append("/home/agilex/ainnovation/cobot_magic/aloha-devel/")
...
class DETRVAE(nn.Module):
    ...
    def forward(self, image, depth_image, robot_state, actions=None, action_is_pad=None):
        ...
        hs = self.transformer(self.query_embed.weight,
                              src, src_pos, None,
                              robot_state_input, self.robot_state_pos.weight,
                              latent_input, self.latent_pos.weight)[-1]
