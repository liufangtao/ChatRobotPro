...
def save_videos(args, dataset_path, img_dict):
    cam_names = list(img_dict.keys())
    for cam_name in cam_names:
        file_name = ''
        is_rgb = True
        if cam_name == 'camera_3_rgb_front':
            file_name = 'camera3_rgb'
            is_rgb = True
        elif cam_name == 'camera_1_rgb_left':
            file_name = 'camera1_rgb'
            is_rgb = True
        elif cam_name == 'camera_2_rgb_right':
            file_name = 'camera2_rgb'
            is_rgb = True
        elif cam_name == 'camera_5_rgb_head':
            file_name = 'camera5_rgb'
            is_rgb = True
        elif cam_name == 'camera_3_depth_front':
            file_name = 'camera3_depth'
            is_rgb = False
        elif cam_name == 'camera_1_depth_left':
            file_name = 'camera1_depth'
            is_rgb = False
        elif cam_name == 'camera_2_depth_right':
            file_name = 'camera2_depth'
            is_rgb = False
        elif cam_name == 'camera_5_depth_head':
            file_name = 'camera5_depth'
            is_rgb = False
        
        save_path = dataset_path + "_" + file_name + ".mp4"
        if is_rgb:
            frames = len(img_dict[cam_name])
            height, width, channels = img_dict[cam_name][0].shape
            fourcc = cv2.VideoWriter_fourcc(*'mp4v')  # 使用 mp4 编码
            video_writer = cv2.VideoWriter(save_path, fourcc, args.frame_rate, (width, height))
            for i in range(frames):
                frame = img_dict[cam_name][i]
            ...
            frames = len(img_dict[cam_name])
            height, width = img_dict[cam_name][0].shape
            fourcc = cv2.VideoWriter_fourcc(*'XVID')
            out = cv2.VideoWriter(save_path, fourcc, args.frame_rate, (width, height), isColor=False)
        
            for i in range(frames):
                out.write(img_dict[cam_name][i])
            out.release()
...
def load_json():
    ...
    with open(os.path.join(bait_type_path, 'metadata_copy_3.json'), 'r', encoding='utf-8') as file:
        data = json.load(file)
...
# 保存数据函数
def save_data(args, timesteps, actions, actions_ee, dataset_path, cur_datetime, user, rate, num_steps, json_data, cur_task):
    # 数据字典
    ...
    while actions:
        ...
        if args.save_video:
            for cam_name in args.camera_names:
                img_dict[cam_name].append(ts.observation['images'][cam_name])
            if args.use_depth_image:
                for depth_cam_name in args.depth_camera_names:
                    img_dict[depth_cam_name].append(ts.observation['images_depth'][depth_cam_name])
        else:
            for cam_name in args.camera_names:
    ...
    if args.save_video:
        ...
        save_videos(args, dataset_path, img_dict)
    ...
    with h5py.File(dataset_path + '.hdf5', 'w', rdcc_nbytes=1024**2*2) as root:
        ...
        # save metadata
        save_metadata(root, cur_datetime, user, rate, num_steps, json_data, cur_task)
...
class RosOperator:
    ...
    def process(self):
        ...
        print("len(actions_ee) :", len(actions_ee))
        return timesteps, actions, actions_ee
...
def main():
    ...
    if args.use_async:
        pool = multiprocessing.Pool(processes=args.process_count)
    ...
    while True:
        ...
        on_process = True
        timesteps, actions, actions_ee = ros_operator.process()
        on_process = False
        
        dataset_dir = os.path.join(args.dataset_dir, user, args.task_name)

        if args.use_step:
            if (len(actions) < cur_task['step_min'] or len(actions_ee) > cur_task['step_max']):
                print("\033[31m\nSave failure, please record %s timesteps of data.\033[0m\n" % args.max_timesteps)
                continue
        else:
            if (len(actions) < cur_task['step_min'] or len(actions_ee) > cur_task['step_max'] + 200):
        ...
        if status.lower() == 'y':
            print('保存中...', dataset_path)
            if args.use_async:
                # save_data_thread = Thread(target=save_data, 
                #                           args=(copy.deepcopy(args), 
                #                                 copy.deepcopy(timesteps), 
                #                                 copy.deepcopy(actions), 
                #                                 copy.deepcopy(actions_ee), 
                #                                 copy.deepcopy(dataset_path),
                #                                 copy.deepcopy(cur_datetime),
                #                                 copy.deepcopy(user),
                #                                 copy.deepcopy(rate),
                #                                 copy.deepcopy(num_steps),
                #                                 copy.deepcopy(json_data),
                #                                 copy.deepcopy(cur_task)))
                # save_data_thread.setDaemon(True)
                # save_data_thread.start()
                pool.apply_async(save_data, (copy.deepcopy(args), 
                                            copy.deepcopy(timesteps), 
                                            copy.deepcopy(actions), 
                                            copy.deepcopy(actions_ee), 
                                            copy.deepcopy(dataset_path),
                ...
                save_data(timesteps, actions, actions_ee, dataset_path)
                print('保存成功')
